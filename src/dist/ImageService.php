...
class ImageService
{
    ...
    public static function copyImage($key, $value)
    {
        ...
        Storage::put($destinationDirectory . '/' . basename($destination),
            file_get_contents(Storage::get($value->original->url)), 'public');
    ...
    public static function uploadImage($key, $value)
    {
        ...
        $file = Storage::putFileAs(ImageService::getUploadStoragePath() . '/' . dirname($destination), $value,
            basename($destination), 'public');
...
//	        $original_name = basename($urn);
    ...
    public static function imageCrop($source, $destination, $x = 0, $y = 0, $width = 1, $height = 1, $quality = 90)
    {
        ...
        try {
            $image = $imagine->open(Storage::get($source));
            if(class_exists('\Imagine\Filter\Basic\Autorotate')){
                $filterAutorotate = new \Imagine\Filter\Basic\Autorotate();
                $filterAutorotate->apply($image);
            }
            $image_source = $image->crop($point, $box)
                ->get(array_last(explode('.', $destination)), array('quality' => $quality));
            Storage::put($destination, $image_source, 'public');
    ...
    public static function resize($source, $destination, $width = 100, $height = null, $crop = false, $quality = 90)
    {
        ...
        try {
            ...
            $image = $imagine->load(Storage::get($source));
            if(class_exists('\Imagine\Filter\Basic\Autorotate')){
                $filterAutorotate = new \Imagine\Filter\Basic\Autorotate();
                $filterAutorotate->apply($image);
            }

            $image_source = $image->thumbnail($size, $mode)
                ->get(array_last(explode('.', $destination)), array('quality' => $quality));
            Storage::put($destination, $image_source, 'public');
